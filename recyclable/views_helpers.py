...
def create_size_classifier_json() -> str:
    ...
    valid_containers = Container.objects.filter(Q(ca=True) &
                                                ( Q(material_type=Container.MaterialType.ALUMINUM) |
                                                  Q(material_type=Container.MaterialType.GLASS) |
                                                  Q(material_type=Container.MaterialType.PLASTIC)
                                                 )
                                                )
    ...
    for c in valid_containers:
        imgs = Image.objects.filter(Q(container=c) &
                                    Q(valid_orientation=True) &
                                    (Q(crush_degree=0) | Q(crush_degree=1)))

        if c.size == ContainerSize.LT_24_OZ:
            lt24_urls += [img.url() for img in imgs]
        elif c.size == ContainerSize.GTE_24_OZ:
            gte24_urls += [img.url() for img in imgs]

    classifier = SizeClassifier(lt24_urls, gte24_urls)
